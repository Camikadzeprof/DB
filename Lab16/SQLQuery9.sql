...
select	   AUDITORIUM.AUDITORIUM			[Аудитория],
	...
	where AUDITORIUM.AUDITORIUM_TYPE Like 'ЛК%' for xml AUTO, root('Список_аудиторий'), elements;*/
...
delete from SUBJECT where SUBJECT.SUBJECT='КГиГ' or SUBJECT.SUBJECT='ОЗИ' or SUBJECT.SUBJECT='МП'*/



--4
 insert into STUDENT(IDGROUP, SNAME, BDAY, INFO)
	values(22, 'Гурский А.А.', '23.08.2001',
	'<студент>
		<паспорт серия="АВ" номер="3112804" дата="19.09.2016" />
		<телефон>+375292008517</телефон>
		<адрес>
			<страна>Беларусь</страна>
			<город>Барановичи</город>
			<улица>50 лет ВЛКСМ</улица>
			<дом>70</дом>
		</адрес>
	</студент>');
select * from STUDENT where SNAME = 'Гурский А.А.';
update STUDENT set INFO = '<студент>
					           <паспорт серия="AB" номер="31128042" дата="19.09.2016" />
							...
							</студент>' where SNAME='Гурский А.А.'
select SNAME[ФИО], INFO.value('(студент/паспорт/@серия)[1]', 'char(2)')[Серия паспорта],
	INFO.value('(студент/паспорт/@номер)[1]', 'varchar(20)')[Номер паспорта],
	INFO.query('/студент/адрес')[Адрес]
		from  STUDENT
			where SNAME = 'Гурский А.А.';      
...
select SName, INFO from STUDENT where SNAME='Гурский А.А.'*/
...
UPDATE STUDENT set IDGROUP=23 where IDGROUP=24;
DEALLOCATE Studs 
...
DECLARE ScrollCur CURSOR LOCAL DYNAMIC SCROLL for 
		SELECT row_number() over (order by SNAME), SNAME from STUDENT where IDGROUP = 6 
OPEN ScrollCur;
...
SELECT SNAME, NOTE from PROGRESS inner join STUDENT 
...
SELECT SNAME, NOTE from PROGRESS join STUDENT
	on PROGRESS.IDSTUDENT = STUDENT.IDSTUDENT
		where NOTE<5
